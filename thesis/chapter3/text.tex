...
% note difficulties involved (ambiguities of integration along LOS as well)
% summarize various other approaches

The differential emission measure distribution is a useful diagnostic for understanding the temperature distribution of the coronal plasma and, by extension, the underlying energy deposition. However, deriving the $\dem$ from observed spectral line or narrowband intensities is difficult for a number of reasons. \autoref{eq:intensity_dem} is an inhomogeneous Fredholm equation of the first kind,
...
In solar astrophysics specifically, a great amount of effort has been focused on methods for solving \autoref{eq:intensity_dem} for the $\dem$ ($f$) given a set of observed EUV and/or X-ray intensities ($g$) using contribution functions or detector responses ($K$) derived from atomic data. These methods include, but are not limited to, Gaussian, multi-Gaussian, and spline fits via $\chi^2$-minimization \citep[e.g.][]{warren_observations_2013,ryan_compatibility_2014,caspi_constraining_2014}, Markov Chain Monte Carlo \citep[MCMC,][]{kashyap_markov-chain_1998}, regularized inversion \citep{hannah_differential_2012,plowman_fast_2013}, sparse basis pursuit \citep{cheung_thermal_2015}, and sparse Bayesian inference \citep{warren_sparse_2017}. Many of these methods have been designed to work well with specific observations and, due to the lack of a unique solution to \autoref{eq:intensity_dem}, tend to all give different $\dem$ for the same set of inputs $I$ \citep[see comparison of 15 different methods by][]{aschwanden_benchmark_2015}. 
