...
\begin{align}\label{eq:potential_bcs}
    &-\mathbf{n}\cdot\nabla\phi = B_z(x,y,0), \\
    &\phi(\mathbf{r})\to0\,\textup{as}\,\mathbf{r}\to\infty,
\end{align}
...
\begin{align}
    ...
    &G(\mathbf{r},\mathbf{r}^\prime) \to 0 \, \textup{as} \, |\mathbf{r}-\mathbf{r}^\prime|\to\infty, \nonumber \\
...
\end{equation}
\citet{schmidt_observable_1964} suggest an ``oblique'' correction to $G$ for cases where the \AR{} is far from disk center and $\mathbf{n}$, the surface normal, is not aligned with unit vector pointing toward the observer, $\bm{\ell}$. In this case, the modified Green's function is,
...
m_ar_resampled = m_ar.resample([50, 50] * u.pixel)
n_z = 50 * u.pixel
w_z = 215 * u.Mm
...
\end{pyblock}
% spell-checker: enable %
This traces 200 fieldlines through the extrapolated volume and returns a list of three-dimensional coordinates and field strengths along the strand for each of the 200 traced fieldlines. By default, synthesizAR places seed points for the fieldline tracing on the lower boundary in areas of relatively high magnetic field strength though this parameter can be adjusted. Additionally, the strands are filtered by  lengths can be specified (as shown above) and only strands 
